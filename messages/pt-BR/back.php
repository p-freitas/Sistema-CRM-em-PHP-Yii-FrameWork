<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message/extract' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    'Confirmation Token' => 'Token de confirmação',
    'E-mail' => '',
    'ID' => '',
    'IP' => '',
    'Refresh routes' => 'Atualizar rotas',
    'Refresh routes (and delete unused)' => 'Atualizar rotas (e remover inúteis)',
    'Routes that are not exists in this application will be deleted. Do not recommended for application with "advanced" structure, because frontend and backend have they own set of routes.' => 'Rotas que não existem nesta aplicação serão deletadas. Não recomendado para aplicações com estrutura "advanced", porque backend e frontend tem suas próprias rotas.',
    'Search route' => 'Procurar rota',
    'Show all' => 'Mostrar todas',
    'Show only selected' => 'Mostrar selecionados',
    'Superadmin' => '',
    'Token' => '',
    'Permissions for role - "{role}"' => 'Permissões da função - "{role}"',
    'Active' => 'Ativo',
    'Are you sure you want to delete this user?' => 'Você tem certeza de que deseja excluir este usuário?',
    'Banned' => 'Banido',
    'Bind to IP' => 'Vincular a IP',
    'Browser' => 'Navegador',
    'Change own password' => 'Alterar própria senha',
    'Change password' => 'Alterar senha',
    'Changing password' => 'Alterando senha',
    'Changing password for user: ' => 'Alterando senha do usuário: ',
    'Child permissions' => 'Permissões filhas',
    'Child roles' => 'Regras filhas',
    'Code' => 'Código',
    'New' => 'Novo',
    'Create' => 'Criar',
    'Created' => 'Criado',
    'Creating permission group' => 'Criando grupo de permissões',
    'Current password' => 'Senha atual',
    'Data' => 'Dados',
    'Delete' => 'Excluir',
    'Deleted' => 'Excluído com sucesso',
    'Description' => 'Descrição',
    'E-mail confirmed' => 'E-mail confirmado',
    'E-mail with activation link has been sent to <b>{email}</b>. This link will expire in {minutes} min.' => 'Um e-mail com o link de ativação foi enviado para <b>{email}</b>. Este link irá expirar em {minutes} minutos.',
    'Edit' => 'Editar',
    'Editing' => 'Editando',
    'Editing permission group' => 'Editando grupo de permissões',
    'Editing permission: ' => 'Editando permissão: ',
    'Editing role: ' => 'Editando função: ',
    'Editing user: ' => 'Editando usuário: ',
    'For example: 123.34.56.78, 168.111.192.12' => 'Por exemplo: 123.34.56.78, 168.111.192.12',
    'Group' => 'Grupo',
    'Inactive' => 'Inativo',
    'Language' => 'Idioma',
    'Login' => 'Login',
    'Name' => 'Nome',
    'OS' => 'Sistema operacional',
    'Password' => 'Senha',
    'Password has been changed' => 'A senha foi alterada',
    'Permission creation' => 'Criação de permissão',
    'Permission groups' => 'Grupos de permissão',
    'Permissions' => 'Permissões',
    'Permissions for role:' => 'Permissões da função:',
    'Registration IP' => 'Registro de IP',
    'Repeat password' => 'Repetir senha',
    'Reset' => 'Redefinir',
    'Role creation' => 'Criação de função',
    'Roles' => 'Funções',
    'Roles and permissions' => 'Funções e permissões',
    'Roles and permissions for user:' => 'Funções e permissões do usuário:',
    'Routes' => 'Rotas',
    'Rule' => 'Regra',
    'Save' => 'Salvar',
    'Saved' => 'Salvo com sucesso',
    'Search' => 'Buscar',
    'Settings for permission' => 'Configurações de permissões',
    'Status' => 'Status',
    'Too many attempts' => 'Muitas tentativas',
    'Type' => 'Tipo',
    'Updated' => 'Atualizado',
    'User' => 'Usuário',
    'User agent' => 'Agente do Usuário',
    'User creation' => 'Criação de usuário',
    'Users' => 'Usuários',
    'Visit Time' => 'Tempo de Visita',
    'Visit log' => 'Log de Visitas',
    'Wrong format. Enter valid IPs separated by comma' => 'Formato incorreto. Insira endereços de IPs válidos e separados por vírgula',
    'Wrong password' => 'Senha incorreta',
    'You can not change own permissions' => 'Você não pode alterar suas próprias permissões',
    
];
